...
ssh -p 5490 root@138.219.42.117 "cd /var/www/aoweb && git pull && cd server && pnpm install && pnpm build && pnpm export-frontend-maps:active && cd ../frontend && pnpm install && pnpm build && cd ../api && pnpm install && pnpm build && pm2 restart all && pm2 save && pm2 list"
