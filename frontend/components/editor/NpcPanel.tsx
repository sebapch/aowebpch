...
import { EditorApiError, fetchNpcTemplate, saveNpcTemplate, searchItemTemplates } from "../../lib/editor/api";
...
function DropListEditor({
    ...
    // Resuelve el nombre de los items ya presentes en la lista (uno por id
    // faltante, cacheado: no se repite para ids ya resueltos).
    useEffect(() => {
        const missingIds = [...new Set(entries.map((entry) => entry.item))].filter((id) => !(id in itemNames));
    ...
    const addItem = (item: ItemTemplateSummary) => {
        ...
        setItemNames((current) => ({ ...current, [item.id]: item.name }));
        setQuery("");
        setResults([]);
    };
